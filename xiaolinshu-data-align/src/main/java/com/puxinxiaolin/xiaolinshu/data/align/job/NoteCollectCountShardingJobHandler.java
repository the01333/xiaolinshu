...
@Component
@Slf4j
public class NoteCollectCountShardingJobHandler {
    @Resource
    ...
    public void noteCollectCountShardingJobHandler() throws Exception {
        ...
        for (; ; ) {
            // 1. 分批次查询 t_data_align_note_collect_count_temp_日期_分片序号, 如一批次查询 1000 条
            ...
            // 2. 对 t_note_collect 笔记收藏表执行 count(*) 操作，获取总数
            noteIds.forEach(noteId -> {
                int collectCount = selectMapper.selectCountFromFollowingTableByUserId(noteId);

                // 3. 更新 t_note_count 表，并更新对应 Redis 缓存
                int count = updateMapper.updateNoteCollectTotalByUserId(noteId, collectCount);
                if (count > 0) {
                    String redisKey = RedisKeyConstants.buildCountNoteKey(noteId);
                    Boolean hasKey = redisTemplate.hasKey(redisKey);
                    if (hasKey) {
                        redisTemplate.opsForHash()
                                .put(redisKey, RedisKeyConstants.FIELD_LIKE_TOTAL, collectCount);
                    }
                }
