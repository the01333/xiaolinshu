...
public class TodayNoteCollectIncrementData2DBConsumer implements RocketMQListener<String> {
    ...
    public void onMessage(String message) {
        ...
        String redisKey = RedisKeyConstants.buildBloomUserNoteCollectListKey(date);
        ...
        Long result = redisTemplate.execute(script, Collections.singletonList(redisKey), noteId);
        // 若布隆过滤器判断不存在（绝对正确）
        if (Objects.equals(result, 0L)) {
            ...
            redisTemplate.execute(bloomAddScript, Collections.singletonList(redisKey), noteId);
        }
    }
