...
@RocketMQMessageListener(
        consumerGroup = MQConstants.TOPIC_COUNT_NOTE_LIKE,
        topic = "xiaolinshu_group_" + MQConstants.TOPIC_COUNT_NOTE_LIKE
)
public class CountNoteLikeConsumer implements RocketMQListener<String> {
    ...
    private void consumeMessage(List<String> messages) {
        ...
        for (Map.Entry<Long, List<CountLikeUnlikeNoteMqDTO>> entry : groupMap.entrySet()) {
            ...
            result.put(entry.getKey(), finalCount);
        }

        log.info("## 【笔记点赞数】聚合后的计数数据: {}", JsonUtils.toJsonString(result));
        
        // 同步 redis
        result.forEach((noteId, score) -> {
            String countNoteKey = RedisKeyConstants.buildCountNoteKey(noteId);
            
            Boolean isExisted = redisTemplate.hasKey(countNoteKey);
            if (isExisted) {    
                redisTemplate.opsForHash()
                        .increment(countNoteKey, RedisKeyConstants.FIELD_LIKE_TOTAL, score);
            }
        });
        
        // 走 MQ, 确保数据入库
        Message<String> message = MessageBuilder.withPayload(JsonUtils.toJsonString(result))
                .build();
